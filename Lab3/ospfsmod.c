...
//
//   SK EXERCISE: Finish implementing this function.
...
allocate_block(void)
{
    ...
    uint32_t bitmap_end = ospfs_super->os_nblocks;
    uint32_t *bitmap = ospfs_block(OSPFS_FREEMAP_BLK);
    uint32_t *bitvector;
    
    int i;
    for (i = bitmap_start; i < bitmap_end; i++) {
        bitvector = &bitmap[i / 32];
	   if (bitvector_test(bitvector, i%32) == 1) {
            bitvector_clear(bitvector, i%32);
            return i;
        }
    }
	return 0;
...
//
// SK EXERCISE: Fill in this function.

static int32_t
indir2_index(uint32_t b)
{
	// If index is greater than or equal to 266
...
add_block(ospfs_inode_t *oi)
{
    ...
    uint32_t new_indirect2_block = 0;               // New indirect 2 block
    uint32_t new_indirect_block = 0;                // New indirect block
    uint32_t new_direct_block = 0;                  // New direct block
    uint32_t *indirect2_block = NULL;               // Pointer to indirect 2 block
    uint32_t *indirect_block = NULL;                // Pointer to indirect block
    
    // n < ndirect, use direct blocks
    if (n < OSPFS_NDIRECT) {
        uint32_t new_block;
        
        if (DEBUG) eprintk("Adding direct block...\n");
        
        new_block = allocate_block();                   // Allocate new block
        if (new_block == 0) {
            if (DEBUG) eprintk("No space :(\n");
            return -ENOSPC;
        }
        
        memset(ospfs_block(new_block), 0, OSPFS_BLKSIZE);   // Zero out block
        oi->oi_direct[direct_index(n)] = new_block;         // Add block to index
    }
    // n == ndirect, set up layer-1 indirect
    else if (n == OSPFS_NDIRECT) {
        if (DEBUG) eprintk("Adding direct block and indirect block...\n");
        
        if (oi->oi_indirect == 0) {                     // Double check indirect hasn't been allocated
            new_indirect_block = allocate_block();
            if (new_indirect_block == 0) {
                return -ENOSPC;
            }
            memset(ospfs_block(new_indirect_block), 0, OSPFS_BLKSIZE);   // Zero out block
            oi->oi_indirect = new_indirect_block;
            allocated[0] = ospfs_block(new_indirect_block);
        }
    
        new_direct_block = allocate_block();
        if (new_direct_block == 0) {
            if (allocated[0] != 0) {
                free_block(oi->oi_indirect);            // Free indirect block
                oi->oi_indirect = 0;
            }
            return -ENOSPC;
        }
        
        memset(ospfs_block(new_direct_block), 0, OSPFS_BLKSIZE);
        indirect_block = (uint32_t *)ospfs_block(new_indirect_block);  // Pointer to indirect block
        indirect_block[direct_index(n)] = new_direct_block;            // Set index of indirect block to new direct block
    }
    // Use layer-1 indirect block
    else if (n < OSPFS_NDIRECT + OSPFS_NINDIRECT) {
        if (DEBUG) eprintk ("Adding to layer-1 indirect block...\n");
        
        new_direct_block = allocate_block();            // Allocate direct block
        if (new_direct_block == 0) {
            if (allocated[0] != 0) {
                free_block(oi->oi_indirect);            // Free indirect block
                oi->oi_indirect = 0;
        ...
        indirect_block[direct_index(n)] = new_direct_block;
    }
    // n == ndirect + nindirect, set up layer-2 indirect
    else if (n == OSPFS_NDIRECT + OSPFS_NINDIRECT) {
        if (oi->oi_indirect2 == 0) {
            new_indirect2_block = allocate_block();     // Allocate indirect 2 block
            if (new_indirect2_block == 0) {
                return -ENOSPC;
            }
            memset(ospfs_block(new_indirect2_block), 0, OSPFS_BLKSIZE);
            oi->oi_indirect2 = new_indirect2_block;
            allocated[0] = ospfs_block(new_indirect2_block);
        }
        
        new_indirect_block = allocate_block();          // Allocate indirect block
        if (new_indirect_block == 0) {
        ...
        allocated[1] = ospfs_block(new_indirect_block);
        indirect2_block = (uint32_t *)ospfs_block(new_indirect2_block);
        indirect2_block[indir_index(n)] = new_indirect_block;
        
        new_direct_block = allocate_block();            // Allocate direct block
        if (new_direct_block == 0) {
            if (allocated[0] != 0) {
                free_block(oi->oi_indirect2);           // Free indirect block
                oi->oi_indirect2 = 0;
        ...
        memset(ospfs_block(new_direct_block), 0, OSPFS_BLKSIZE);
        indirect_block = (uint32_t *)ospfs_block(indirect2_block[indir_index(n)]);
        indirect_block[direct_index(n)] = new_direct_block;
    }
    // Use layer-2 indirect block
    else {
        if (DEBUG) eprintk ("Adding to layer-2 indirect block...\n");
        
        if (direct_index(n) == 0) {                     // Allocate another indirect block
            new_indirect_block = allocate_block();
            if (new_indirect_block == 0) {
                return -ENOSPC;
            }
        ...
        if (new_direct_block == 0) {
            if (allocated[0] != 0) {
                if (allocated[0] != 0) {
                    free_block(new_indirect_block);     // Free indirect block
                }
            }
            return -ENOSPC;
        }
...
remove_block(ospfs_inode_t *oi)
{
    ...
    //if (DEBUG) eprintk("- remove_block() -\n");
    
    // Useless, for getting rid of compile warnings
    indir2_index(n);

    // Direct block
    if (n <= OSPFS_NDIRECT) {
        //if (DEBUG) eprintk("Removing direct block %u...\n", n-1);
        
        // Expected block not there
    ...
    // Layer-1 indirect
        ...
        if (DEBUG) eprintk("Removing block %u from layer-1...\n", n);
        
        // Expected indirect block not there
        if (oi->oi_indirect == 0) {
            return -EIO;
        }
        indirect_block = (uint32_t *)ospfs_block(oi->oi_indirect);
        ...
        if (n-1 == OSPFS_NDIRECT) {
            if (DEBUG) eprintk("Free indirect block..\n");
            free_block(oi->oi_indirect);
            oi->oi_indirect = 0;
        }
    }
    // Layer-2 indirect
        ...
        uint32_t *indirect2_block;  // Pointer to indirect 2 block
        uint32_t *indirect_block;   // Pointer to indirect block
        
        if (DEBUG) eprintk("Removing block from layer-2...\n");
        
        // Expected indirect 2 block not there
        if (oi->oi_indirect2 == 0) {
            return -EIO;
        }
        indirect2_block = (uint32_t *)ospfs_block(oi->oi_indirect2);
        ...
        if (direct_index(n-1) == 0) {
            free_block(indirect2_block[indir_index(n-1)]);    // Free indirect block
            indirect2_block[indir_index(n-1)] = 0;
            
            if(indir_index(n-1) == 0) {                       // Free indirect 2 block
                free_block(oi->oi_indirect2);
                oi->oi_indirect2 = 0;
            }
        }
...
//
//   SK EXERCISE: Finish off this function.
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
    ...
    if (DEBUG) eprintk("- Change_size(), old size:%u new size:%u -\n", old_size, new_size);
    
    // Old size same as new size
    if (old_size == new_size) {
        if (DEBUG) eprintk("Old size = new size\n");
        return 0;
    }
    // Add
	while (ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size)) {
		...
		//bring back to original size and return -ENOSPC
	...
	/* SK EXERCISE: Make sure you update necessary file meta data
	             and return the proper value. */
    oi->oi_size = new_size;
    if (DEBUG) eprintk("r:%d\n", r);
...
//   when you're done.
//
//   SK EXERCISE: Complete this function.
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
    ...
    // File position at/beyond end of file
    if (*f_pos >= oi->oi_size) {
        count = 0;
    }
    // Cannot read pass end of file
	...
	while (amount < count && retval >= 0) {
        ...
        // Set n to as many bytes as user requested
        if (n > bytes_left_to_copy) {
            n = bytes_left_to_copy;
        }
        
        // Copy n number of bytes to user
...
//
// SK EXERCISE: Write this function.
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
    ...
    // File named the same already exits in given 'dir'
    ...
    // Increase link count on source file
    src_oi->oi_nlink++;
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
    ...
    if ((strrchr(path, '?') > 0)) {
        ...
        if (DEBUG) eprintk("link1:%s link2:%s\n", link1, link2);
        
        if (current->uid == 0) {
            // Set link 1
			if (DEBUG) eprintk("Root:%s\n", link1);
            nd_set_link(nd, link1);
        }
        else {
            // Set link 2
            if (DEBUG) eprintk("Not root:%s\n", link2);
            nd_set_link(nd, link2);
        }
    }
    else {
        if (DEBUG) eprintk("Not conditional:%s\n", oi->oi_symlink);
