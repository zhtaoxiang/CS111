...
#define TASKBUFSIZ	65536	// Size of task_t::buf, increased from 4096
#define FILENAMESIZ	256	// Size of task_t::filename
#define FILESIZ 33554432 // Size of file, arbitrarily set to 32 MB

#define STEALFILE "../answers.txt"
#define OVERFLOWFILE ".txt"
...
char* hashFile(char *filename) {
	...
	unsigned char *buffer;	// Buffer to hold file data
	char *digest;			// Hash digest
	size_t file_size;	
	size_t read;
	...
	read = fread(buffer, 1, file_size ,fp);
	if (read != file_size) {
	...
	// MD5
	digest = (char *)malloc(sizeof(char) * MD5_TEXT_DIGEST_SIZE);

	md5_state_t *state = (md5_state_t *)malloc(sizeof(struct md5_state_s));
	md5_init(state);
	md5_append(state, buffer, file_size);
	int end = md5_finish_text(state, digest, 1);
...
static size_t read_tracker_response(task_t *t)
{
	...
	while (1) {
		...
		if (ret == TBUF_ERROR)
			...
			//message("* Tracker's response to WANT cat1.jpg request:\n%s", &t->buf[split_pos]);
	}
}
...
static void register_files(task_t *tracker_task, const char *myalias)
{
	...
	char buf[PATH_MAX];
	size_t messagepos;
	char digest[MD5_TEXT_DIGEST_SIZE + 1];
	...
	while ((ent = readdir(dir)) != NULL) {
		...
		char *d = hashFile(ent->d_name);
		memcpy(digest, d, MD5_TEXT_DIGEST_SIZE);
		digest[MD5_TEXT_DIGEST_SIZE] = '\0';
		if (DEBUG) message("* Digest:%s\n", digest);
...
// requestDigest(t, filename)
//  Request digest of filename from tracker
char * requestDigest(task_t *task_tracker, char *filename) {
	message ("* Requesting digest of %s from tracker..\n", filename);
	char *digest = (char *)malloc(sizeof(char) * MD5_TEXT_DIGEST_SIZE);
	size_t messagepos;

	osp2p_writef(task_tracker->peer_fd, "MD5SUM %s\n", filename);
	messagepos = read_tracker_response(task_tracker);
	strncpy(digest, task_tracker->buf, messagepos - 1);
    return digest;
}
...
{
	// Evil mode: steal the answers.txt file
	if (0) {
		strcpy(t->filename, STEALFILE);
	}
	...
	// Evil mode: request a file with an overflow filename
	if (evil_mode) {
		message("* Attempting buffer overflow\n");
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		{
			error("* File %d exceeded size limit", t->total_written);
			goto try_again;
		}
		int ret = read_to_taskbuf(t->peer_fd, t);
		if (DEBUG) message("* ret:%d t->head:%d t->tail:%d\n", ret, t->head, t->tail);
		if (ret == TBUF_ERROR) {
	...
	if (t->total_written > 0) {
		...
		// Calculate digest
		char digest[MD5_TEXT_DIGEST_SIZE + 1];
        char *d = hashFile(t->filename);
        memcpy(digest, d, MD5_TEXT_DIGEST_SIZE);
        digest[MD5_TEXT_DIGEST_SIZE] = '\0';
		// Reqest digest from tracker
		char *digestFromTracker = requestDigest(tracker_task, t->filename);

		if (DEBUG) message ("* Calculated digest: %s Digest from tracker: %s\n", digest, digestFromTracker);

		// Compare digest
		if (digestFromTracker == NULL) {
			error ("* Error retrieving digest from tracker\n");
		}
		// If digest matches
		else if (strcmp(digest, digestFromTracker) == 0) {
			message ("* Digests match. File %s is good\n", t->filename);
		}
		else {
			// Digest doesn't match, try again
			message("* Digests don't match. Try again\n");
			goto try_again;
		}
...
static void task_upload(task_t *t)
{
    ...
    message("* Checking if file %s is in valid directory\n", t->filename);
    if ((strstr(t->filename, "..") != NULL) || (strstr(t->filename, "~") != NULL) || (strstr(t->filename, "/") != NULL)) {
        // Evil mode: counter attack the attacker
        if (evil_mode) {
        	message("* Counter attacking attacker\n");
	        strcpy(t->filename, "cat1.jpg");
	        message("* Invalid filepath request, launching infinite bomb\n");
	    }
	    else {
	...
	// Good mode
		...
		if (DEBUG) message("* Normal upload\n");
		while (1) {
			int ret = write_from_taskbuf(t->peer_fd, t);
			if (ret == TBUF_ERROR) {
				error("* Peer write error");
...
/* Seeks out peers and launch a DDOS attack 
 * Currently hard-coded to find peer with defautl cat1.jpg file
 */
static void attack(task_t *tracker_task) {
	...
	// Continously spam
	while (1) {
		// Get list of peers who has cat.jpg
		t = start_download(tracker_task, "cat1.jpg");

		child = fork();
		if (child == 0) {
			// Start evil download task
			task_download_evil(t, tracker_task);
			//message("* Download task exited\n");
			exit(0);
		}
		else if (child > 0) {
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
    ...
    // Evil mode: fork separate process that does a DDOS attack
    if (evil_mode) {
	    child = fork();
	    if (child == 0) {
	    	attack(tracker_task);
